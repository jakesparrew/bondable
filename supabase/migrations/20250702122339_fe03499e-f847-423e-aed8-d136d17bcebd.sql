...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_recipient_status 
ON public.messages (recipient_id, status) 
WHERE status != 'read';

-- Add index for better performance on real-time message queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_updated_at 
ON public.messages (updated_at DESC);

-- Optimize conversation queries with better indexing
